...
def load_results(results_file: str = "experiments/score_distribution_analysis/results/segment_score_distributions.json"):
...
def analyze_cohesion_scores(cohesion_results):
    ...
    for domain in df['domain'].unique():
        ...
        print(f"  {domain}: mean={domain_data['cohesion_score'].mean():.3f}, "
    ...
    for size in sorted(df['segment_size'].unique()):
        ...
        print(f"  {size} years: mean={size_data['cohesion_score'].mean():.3f}, "
    ...
    if 'temporal_position' in df.columns:
        ...
        for pos in df['temporal_position'].unique():
            ...
            print(f"  {pos}: mean={pos_data['cohesion_score'].mean():.3f}, "
...
def analyze_separation_scores(separation_results):
    ...
    for domain in df['domain'].unique():
        domain_data = df[df['domain'] == domain]
        print(f"  {domain}: mean={domain_data['separation_score'].mean():.3f}, "
              f"std={domain_data['separation_score'].std():.3f}, "
              f"n={len(domain_data)}")
    
    # By temporal distance
    print("\nBy Temporal Distance:")
    df['temporal_distance_cat'] = pd.cut(df['temporal_distance'], 
    ...
    for cat in df['temporal_distance_cat'].unique():
        ...
        print(f"  {cat} years: mean={cat_data['separation_score'].mean():.3f}, "
    ...
    for has_overlap in [True, False]:
        ...
        print(f"  {label}: mean={overlap_data['separation_score'].mean():.3f}, "
...
def correlation_analysis(cohesion_df, separation_df):
    ...
    cohesion_corr = cohesion_df[['cohesion_score', 'segment_size', 'total_papers', 
                               'total_citations', 'avg_papers_per_year']].corr()
    print(cohesion_corr['cohesion_score'].sort_values(ascending=False))
    
    # Separation correlations
    print("\nSeparation Score Correlations:")
    separation_corr = separation_df[['separation_score', 'temporal_distance', 
                                   'temporal_overlap', 'seg1_size', 'seg2_size']].corr()
...
def comparative_analysis(cohesion_df, separation_df):
    ...
    # Range analysis
    print(f"\nRange Analysis:")
    print(f"Cohesion range: [{cohesion_df['cohesion_score'].min():.3f}, {cohesion_df['cohesion_score'].max():.3f}]")
    print(f"Separation range: [{separation_df['separation_score'].min():.3f}, {separation_df['separation_score'].max():.3f}]")
    
    # Quartile analysis
    print(f"\nQuartile Analysis:")
    print(f"Cohesion quartiles: {cohesion_df['cohesion_score'].quantile([0.25, 0.5, 0.75]).values}")
    print(f"Separation quartiles: {separation_df['separation_score'].quantile([0.25, 0.5, 0.75]).values}")
