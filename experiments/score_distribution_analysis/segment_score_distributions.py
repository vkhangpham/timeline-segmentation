...
from core.data.data_processing import load_domain_data, create_academic_periods_from_segments
from core.optimization.objective_function import evaluate_period_cohesion, evaluate_period_separation
from core.utils.config import AlgorithmConfig
from core.utils.logging import get_logger
from core.data.data_models import AcademicYear, AcademicPeriod
...
class SegmentSampler:
    ...
    def load_domain_data(self, domain_name: str) -> Tuple[bool, List[AcademicYear], str]:
    ...
    def create_stratified_segments(self, academic_years: List[AcademicYear], 
                                 num_samples: int, 
                                 min_segment_size: int = 3,
                                 max_segment_size: int = 15) -> List[Tuple[int, int]]:
        ...
        segments = []
        samples_per_stratum = max(1, num_samples // (len(size_strata) * len(temporal_strata)))
        ...
        while len(segments) < num_samples:
            additional_segments = self._sample_random_segments(
                years, min_segment_size, max_segment_size, 
    ...
    def _create_size_strata(self, min_size: int, max_size: int) -> List[Tuple[int, int]]:
    ...
    def _sample_stratum(self, years: List[int], size_stratum: Tuple[int, int], 
                       temporal_stratum: str, num_samples: int) -> List[Tuple[int, int]]:
    ...
    def _sample_random_segments(self, years: List[int], min_size: int, max_size: int, 
...
class ScoreDistributionAnalyzer:
    ...
    def analyze_cohesion_scores(self, domain_name: str, 
        ...
        for start_year, end_year in segments:
            try:
                ...
                result = {
                    ...
                    'avg_papers_per_year': period.total_papers / len(period.academic_years) if period.academic_years else 0,
                    'temporal_position': self._get_temporal_position(start_year, end_year, academic_years)
                }
                ...
                if self.verbose:
                    self.logger.warning(f"Failed to analyze cohesion for {start_year}-{end_year}: {e}")
                continue
        
        return cohesion_results
    
    def analyze_separation_scores(self, domain_name: str,
                                 academic_years: List[AcademicYear],
        ...
        for start_year, end_year in segments:
            try:
                ...
                if self.verbose:
                    self.logger.warning(f"Failed to create period for {start_year}-{end_year}: {e}")
                continue
        ...
        sampled_pairs = []
        for _ in range(min(num_pairs, len(segment_keys) * (len(segment_keys) - 1) // 2)):
        ...
        for (seg1_start, seg1_end), (seg2_start, seg2_end) in sampled_pairs:
            try:
                ...
                if self.verbose:
                    self.logger.warning(f"Failed to analyze separation for {seg1_start}-{seg1_end} vs {seg2_start}-{seg2_end}: {e}")
                continue
        
        return separation_results
    
    def _get_temporal_position(self, start_year: int, end_year: int, 
                              academic_years: List[AcademicYear]) -> str:
    ...
    def _calculate_temporal_distance(self, seg1: Tuple[int, int], 
                                   seg2: Tuple[int, int]) -> int:
    ...
    def _calculate_temporal_overlap(self, seg1: Tuple[int, int], 
                                   seg2: Tuple[int, int]) -> int:
...
class VisualizationGenerator:
    ...
    def __init__(self, output_dir: str = "experiments/score_distribution_analysis/results"):
        self.output_dir = output_dir
        os.makedirs(output_dir, exist_ok=True)
        
    def plot_cohesion_distributions(self, cohesion_results: List[Dict], 
        ...
        axes[0, 0].axvline(df['cohesion_score'].mean(), color='red', linestyle='--', 
        ...
        if 'domain' in df.columns and df['domain'].nunique() > 1:
            for domain in df['domain'].unique():
                ...
                axes[0, 1].hist(domain_data['cohesion_score'], bins=30, alpha=0.6, 
                               label=domain, edgecolor='black')
        ...
        if 'temporal_position' in df.columns:
            sns.boxplot(data=df, x='temporal_position', y='cohesion_score', ax=axes[1, 1])
    ...
    def plot_separation_distributions(self, separation_results: List[Dict],
        ...
        axes[0, 0].set_ylabel('Frequency')
        axes[0, 0].axvline(df['separation_score'].mean(), color='red', linestyle='--',
        ...
        if 'domain' in df.columns and df['domain'].nunique() > 1:
            for domain in df['domain'].unique():
                domain_data = df[df['domain'] == domain]
                axes[1, 1].hist(domain_data['separation_score'], bins=30, alpha=0.6,
                               label=domain, edgecolor='black')
    ...
    def plot_combined_analysis(self, cohesion_results: List[Dict], 
        ...
        axes[0, 0].hist(cohesion_df['cohesion_score'], bins=40, alpha=0.7, 
                       label='Cohesion', edgecolor='black')
        axes[0, 0].hist(separation_df['separation_score'], bins=40, alpha=0.7, 
                       label='Separation', edgecolor='black')
        ...
        axes[1, 0].scatter(cohesion_df['total_papers'], cohesion_df['cohesion_score'], alpha=0.6)
        ...
        axes[1, 1].scatter(separation_df['temporal_distance'], separation_df['separation_score'], alpha=0.6)
...
def main():
    ...
    domains = [
        "applied_mathematics", "art", "computer_vision", "deep_learning", 
        "machine_learning", "machine_translation", "natural_language_processing"
    ]
    
    # Experiment parameters - Large scale experiment  
    K_SAMPLES = 500  # Number of segments per domain (500 x 7 = 3500 total)
    SEPARATION_PAIRS = 500  # Number of separation pairs per domain (500 x 7 = 3500 total)
    VERBOSE = True
    ...
    for domain in domains:
        ...
        cohesion_results = analyzer.analyze_cohesion_scores(domain, academic_years, segments)
    ...
    output_path = os.path.join(visualizer.output_dir, "segment_score_distributions.json")
    ...
    if all_cohesion_results:
        ...
        print(f"Cohesion scores: mean={np.mean(cohesion_scores):.3f}, "
    ...
    if all_separation_results:
        ...
        print(f"Separation scores: mean={np.mean(separation_scores):.3f}, "
