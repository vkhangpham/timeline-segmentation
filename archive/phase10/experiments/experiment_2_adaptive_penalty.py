...
class AdaptivePenaltyExperiment:
    ...
    def __init__(self):
        # Phase-10 focus: restrict to five primary technical domains
        self.domains = [
    ...
    def _run_fixed_penalty_experiment(self, domain_data: DomainData, penalty_value: float) -> Tuple:
        ...
        original_estimate_penalty = getattr(ssd, 'estimate_optimal_penalty', None)
        
        if original_estimate_penalty is not None:
        ...
        try:
            ...
            # Two-signal architecture: citation + direction, no semantic
            shift_signals, _ = detect_shift_signals(
                domain_data,
    ...
    def create_comprehensive_visualizations(self, results_df: pd.DataFrame) -> None:
        ...
        output_dir = Path("../results/experiment_2_visualizations")
...
def main():
    """Run the comprehensive penalty comparison experiment"""
    print("🔬 Adaptive Penalty Validation Experiment")
    print("=" * 60)
