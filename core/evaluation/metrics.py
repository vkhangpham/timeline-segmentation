...
def calculate_segment_f1(
    ...
    # Calculate precision and recall
    precision = (
        matched_predicted / len(predicted_segments) if predicted_segments else 0.0
    )
    recall = (
        matched_ground_truth / len(ground_truth_segments)
...
def calculate_f1_score_between_methods(
    method1_boundaries: List[int], method2_boundaries: List[int], tolerance: int = 2
) -> Dict[str, float]:
    ...
    boundary_precision = (
        boundary_matches / len(method1_boundaries) if method1_boundaries else 0.0
    )
    boundary_recall = (
        boundary_matches / len(method2_boundaries) if method2_boundaries else 0.0
    )
    boundary_f1 = (
