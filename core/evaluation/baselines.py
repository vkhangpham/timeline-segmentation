...
def create_gemini_baseline(
    ...
    if verbose:
        logger.info(f"Gemini baseline raw objective score: {obj_result.final_score:.3f}")
...
def create_manual_baseline(
    ...
    if verbose:
        logger.info(f"Manual baseline raw objective score: {obj_result.final_score:.3f}")
...
def create_fixed_year_baseline(
    ...
    if verbose:
        logger.info(f"{baseline_name} baseline raw objective score: {obj_result.final_score:.3f}")
        logger.info(f"{baseline_name} baseline penalty applied: {penalty:.3f}")
        logger.info(f"{baseline_name} baseline final objective score: {final_score:.3f}")
