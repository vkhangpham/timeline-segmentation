...
def save_evaluation_result(evaluation_result: ComprehensiveEvaluationResult, verbose: bool = False):
...
def display_evaluation_summary(evaluation_result: ComprehensiveEvaluationResult, verbose: bool = False):
    ...
    print(f"\nALGORITHM RESULT:")
    print(f"-" * 20)
    print(f"Objective Score: {alg_result.objective_score:.3f} (Raw: {alg_result.raw_objective_score:.3f}, Penalty: {alg_result.penalty:.3f})")
...
def display_cross_domain_analysis(domain_results: Dict[str, Optional[object]], verbose: bool = False):
    ...
    for domain_name, result in domain_results.items():
        ...
        method_auto_metrics["Algorithm"]["boundary_precision"].append(auto_metrics.boundary_precision)
        method_auto_metrics["Algorithm"]["boundary_recall"].append(auto_metrics.boundary_recall)
        method_auto_metrics["Algorithm"]["segment_f1"].append(auto_metrics.segment_f1)
        method_auto_metrics["Algorithm"]["segment_precision"].append(auto_metrics.segment_precision)
        method_auto_metrics["Algorithm"]["segment_recall"].append(auto_metrics.segment_recall)
        ...
        for i, method_boundaries_single in enumerate(boundaries_list):
            if i < len(manual_boundaries):
                ...
                f1_result = calculate_f1_score_between_methods(
                    method_boundaries_single, 
                    manual_boundaries_single, 
                    tolerance=2
                )
    ...
    for i, (method, boundary_f1) in enumerate(boundary_f1_rankings):
        ...
        print(f"{i+1}. {method}: Boundary F1={boundary_f1:.3f} (±{std_f1:.3f}), Segment F1={segment_f1:.3f} (±{segment_f1_std:.3f})")
    ...
    if "Algorithm" in method_avg_scores:
        ...
        algo_rank = next((i+1 for i, (method, _) in enumerate(sorted_methods) if method == "Algorithm"), None)
        print(f"Algorithm Rank (Objective): #{algo_rank} ({algo_score:.3f})")
        
        # Algorithm F1 rank
        algo_f1_rank = next((i+1 for i, (method, _) in enumerate(boundary_f1_rankings) if method == "Algorithm"), None)
    ...
    save_cross_domain_results(domain_results, method_avg_scores, auto_metrics_avg, method_f1_avg, verbose)
