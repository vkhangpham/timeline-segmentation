...
def _display_segmentation_summary(results: Dict[str, Any], verbose: bool = False) -> None:
    """
...
def _display_full_analysis_summary(results: Dict[str, Any], verbose: bool = False) -> None:
    """
    ...
    for i, period in enumerate(periods):
        ...
        year_span = end - start + 1
        logger.info(f"  {i+1}. {start}-{end}: {topic} (conf: {conf:.3f}, {year_span} years)")
...
def format_timeline_narrative(timeline_result) -> str:
    ...
    """
    if hasattr(timeline_result, 'narrative_evolution') and timeline_result.narrative_evolution:
...
def format_segmentation_details(segmentation_results: Dict[str, Any]) -> str:
    ...
    details = []
    details.append(f"Segmentation Method: {segmentation_results.get('algorithm_config', {}).get('segmentation_method', 'boundary_based')}")
