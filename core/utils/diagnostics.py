    ...
    for academic_year in academic_years:
        if academic_year.year in recent_years and academic_year.keyword_frequencies:
            for keyword, freq in academic_year.keyword_frequencies.items():
                emerging_terms[keyword].append((academic_year.year, freq, academic_year.paper_count))
...
def compute_adaptive_thresholds(
    ...
    thresholds = {
        ...
        "median_plus_mad": np.median(scores_array) + np.median(np.abs(scores_array - np.median(scores_array)))
    }
...
def generate_keyword_evolution_report(
    ...
    for focus_year in focus_years:
        ...
        evolution_report[focus_year] = {
            ...
            "keyword_frequencies": dict(focus_ay.keyword_frequencies) if focus_ay.keyword_frequencies else {}
        }
