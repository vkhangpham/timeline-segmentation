...
from .change_detection import detect_changes, create_improved_segments_with_confidence
...
def timeline_analysis(
    ...
    narrative_parts = []
    for pc in merged_period_characterizations[:3]:  # First 3 periods
        narrative_parts.append(f"{pc.period[0]}-{pc.period[1]}: {pc.topic_label}")
    
    narrative_evolution = " → ".join(narrative_parts)
    if len(merged_period_characterizations) > 3:
        narrative_evolution += f" ... ({len(merged_period_characterizations)} total periods)"
...
def run_change_detection(domain_name: str) -> Tuple[Optional[Dict], Optional[ChangeDetectionResult]]:
    """
    ...
    Returns:
        Tuple of (segmentation_results_dict, change_detection_result) if successful, (None, None) otherwise
    """
    ...
    change_result = detect_changes(domain_data)
    ...
    results = {
        ...
        'method_details': {
            ...
            'methods_used': ['enhanced_shift_signal']
        }
    }
