...
# ShiftSignal model removed - using simple years now
...
def extract_paradigm_shifts_from_manual(
    periods_data: Tuple[Dict[str, Any], ...],
) -> Tuple[int, ...]:
    """
...
def extract_paradigm_shifts_from_gemini(
    periods_data: Tuple[Dict[str, Any], ...],
) -> Tuple[int, ...]:
    """
...
def analyze_temporal_coverage(
    periods_data: Tuple[Dict[str, Any], ...], reference_type: str
) -> Dict[str, Any]:
    """
...
def analyze_period_characteristics(
    periods_data: Tuple[Dict[str, Any], ...], reference_type: str
) -> Dict[str, Any]:
    """
...
def calculate_tolerance_metrics(
    detected_years: Tuple[int, ...], reference_years: Tuple[int, ...], tolerance: int
) -> Dict[str, float]:
    """
    ...
    true_positives = sum(
        ...
        if any(
            abs(detected_year - ref_year) <= tolerance for ref_year in reference_years
        )
    )
    ...
    matched_ref = sum(
        ...
        if any(
            abs(ref_year - detected_year) <= tolerance
            for detected_year in detected_years
        )
    )
...
def evaluate_shifts_against_reference(
    detected_shifts: Tuple[int, ...],
    ...
    detected_years = tuple(detected_shifts)  # Already integers
    ...
    count_accuracy = max(
        0.0,
        1.0
        - abs(len(detected_years) - len(reference_shifts))
        / max(len(reference_shifts), 1),
    )
...
def process_reference_data(
    ref_data: Dict[str, Any], reference_type: str
) -> ReferenceData:
    """
    ...
    period_characteristics = analyze_period_characteristics(
        periods_tuple, reference_type
    )
...
def load_reference_data_from_files(
    ...
    if reference_type not in ("manual", "gemini"):
        raise ValueError(
            f"Invalid reference type: {reference_type}. Must be 'manual' or 'gemini'."
        )
    ...
    if not matching_files:
        raise FileNotFoundError(
            f"No {reference_type} reference files found in {references_dir}. Pattern: {file_pattern}"
        )
...
def get_reference_summary(
    manual_refs: Dict[str, ReferenceData], gemini_refs: Dict[str, ReferenceData]
) -> Dict[str, Any]:
    """
    ...
    for domain, data in manual_refs.items():
        summary["manual_references"][domain] = {
            ...
            "avg_period_length": data.period_characteristics.get(
