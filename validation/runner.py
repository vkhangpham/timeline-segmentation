...
from validation.core import (
    ...
    evaluate_shifts_against_reference,
)
...
class ValidationFramework:
    ...
    def run_validation(self) -> OverallResults:
        ...
        for domain in self.available_domains:
            ...
            print(
                f"    F1={result.f1_score_2yr:.3f}, P={result.precision_2yr:.3f}, R={result.recall_2yr:.3f}"
            )
    ...
    def _evaluate_domain(self, domain_name: str) -> ValidationResult:
        ...
        config = AlgorithmConfig.from_config_file(domain_name=domain_name)

        success, academic_years, error_message = load_domain_data(
            domain_name=domain_name, algorithm_config=config, verbose=False
        )

        if not success:
            raise RuntimeError(f"Error loading {domain_name}: {error_message}")

        reference = self.reference_data[domain_name]

        # Run algorithm with domain-specific optimized parameters
        detected_shift_academic_years = detect_boundary_years(
        ...
        # Extract years from AcademicYear objects for evaluation
        detected_shifts = [ay.year for ay in detected_shift_academic_years]

        # Evaluate performance
        evaluation = evaluate_shifts_against_reference(
            detected_shifts, reference.paradigm_shifts
        )
    ...
    def _calculate_overall_metrics(
        self, domain_results: Dict[str, ValidationResult]
    ) -> OverallResults:
        """Calculate weighted overall metrics."""

        # Calculate weighted averages by number of reference shifts
        total_shifts = sum(
            len(result.reference_shifts) for result in domain_results.values()
        )
        ...
        weighted_f1 = (
            sum(
                result.f1_score_2yr * len(result.reference_shifts)
        ...
        weighted_precision = (
            sum(
                result.precision_2yr * len(result.reference_shifts)
                for result in domain_results.values()
            )
            / total_shifts
        )
        weighted_recall = (
            sum(
                result.recall_2yr * len(result.reference_shifts)
                for result in domain_results.values()
            )
            / total_shifts
        )
    ...
    def _save_results(self, results: OverallResults, validation_time: float):
        ...
        output_file = (
            output_dir / f"validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
