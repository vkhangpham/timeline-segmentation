...
def run_merging():
    ...
    if st.session_state.final_periods is None:
        with st.spinner("Merging similar periods..."):
            ...
            merge_threshold = getattr(st.session_state, 'merge_similarity_threshold', 0.6)
...
def show_merging_analysis(initial_periods, final_periods):
    ...
    initial_avg_duration = sum(p.end_year - p.start_year + 1 for p in initial_periods) / len(initial_periods) if initial_periods else 0
    final_avg_duration = sum(p.end_year - p.start_year + 1 for p in final_periods) / len(final_periods) if final_periods else 0
    
    # Calculate duration standard deviation
    initial_durations = [p.end_year - p.start_year + 1 for p in initial_periods]
    final_durations = [p.end_year - p.start_year + 1 for p in final_periods]
    
    initial_duration_std = (sum((d - initial_avg_duration) ** 2 for d in initial_durations) / len(initial_durations)) ** 0.5 if initial_durations else 0
    final_duration_std = (sum((d - final_avg_duration) ** 2 for d in final_durations) / len(final_durations)) ** 0.5 if final_durations else 0
    ...
    if len(final_periods) < len(initial_periods):
        st.write("Based on keyword similarity and temporal adjacency, the following merging occurred:")
        ...
        for i, (final_start, final_end) in enumerate(final_years):
            ...
            if len(contributing_periods) > 1:
                st.write(f"- Final Period {i+1} ({final_start}-{final_end}) ← Initial Periods {contributing_periods}")
        ...
        if initial_periods:
            ...
            for i, period in enumerate(initial_periods):
                initial_data.append({
                    ...
                    'Topic': (period.topic_label[:25] + "..." if period.topic_label and len(period.topic_label) > 25 else period.topic_label) or "N/A"
                })
...
def show_merging():
    ...
    with col1:
        ...
        st.write("⚠️ **Note:** This stage may also include optimization steps that can take time.")
    with col2:
    ...
    tab1, tab2, tab3 = st.tabs(["🔍 Merging Analysis", "📋 Period Comparison", "📖 Algorithm Info"])
    ...
    with tab3:
        ...
        **Benefits:**
        ...
        if "Merging" in st.session_state.timing_data:
            st.write(f"**Last Run Time:** {st.session_state.timing_data['Merging']:.2f} seconds")
