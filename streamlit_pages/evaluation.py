...
def display_baseline_comparison(results: Dict[str, Any]):
    ...
    if isinstance(baseline_results, list):
        ...
        for baseline_name, baseline_data in baseline_results.items():
            scores.append((baseline_name.title(), baseline_data.get('objective_score', 0)))
        ...
        with st.expander("📊 Baseline Details"):
            ...
            if isinstance(baseline_results, list):
                for baseline_data in baseline_results:
                    ...
                    st.write(f"- Objective Score: {baseline_data.get('objective_score', 0):.3f}")
                    st.write(f"- Segments: {baseline_data.get('num_segments', 0)}")
                    
                    boundary_years = baseline_data.get('boundary_years', [])
                    if boundary_years:
                        st.write(f"- Boundary Years: {', '.join(map(str, boundary_years))}")
                    st.write("---")
            else:
                for baseline_name, baseline_data in baseline_results.items():
                    st.write(f"**{baseline_name.title()} Baseline:**")
...
def create_baseline_segments(boundary_years: List[int], domain_name: str) -> List[Dict[str, Any]]:
    ...
    for i in range(len(boundary_years) - 1):
        ...
        end_year = boundary_years[i + 1] - 1 if i < len(boundary_years) - 2 else boundary_years[i + 1]
        
        segments.append({
            'start_year': start_year,
            'end_year': end_year,
            'period_name': f"{start_year}-{end_year}",
            'top_keywords': [f"{domain_name} research", "academic literature", "scholarly work"],  # Generic keywords for baselines
...
def plot_detailed_timeline(timeline_data: Dict[str, Any], title: str, color: str, y_position: float) -> go.Figure:
...
def show_comprehensive_timeline_visualization(results: Dict[str, Any], domain_name: str):
    ...
    st.write("Detailed visualization of all timeline segmentation methods with keywords for each period")
        ...
        with st.expander(f"🔍 {timeline['name']} Timeline Details"):
            ...
            for i, period in enumerate(periods):
                period_data.append({
                    ...
                    'Papers': f"{period.get('total_papers', 0):,}" if period.get('total_papers', 0) > 0 else "N/A",
            ...
            for i, period in enumerate(periods):
                keywords = period.get('top_keywords', [])
                if keywords:
                    st.write(f"**Period {i+1} ({period.get('start_year', 0)}-{period.get('end_year', 0)}):**")
...
def show_timeline_visualization(results: Dict[str, Any]):
    ...
    if isinstance(baseline_results, list):
        for i, baseline_data in enumerate(baseline_results):
            baseline_name = baseline_data.get('baseline_name', '')
            if baseline_name in ['5-year', '10-year']:  # Skip these for cleaner visualization
                continue
