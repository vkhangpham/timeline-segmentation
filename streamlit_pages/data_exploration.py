...
def create_domain_statistics_cards(academic_years):
    ...
    year_span = max(ay.year for ay in academic_years) - min(ay.year for ay in academic_years) + 1
...
def show_paper_details_table(academic_years):
    ...
    if papers_data:
        ...
        with col2:
            year_filter = st.selectbox("Filter by Year", ["All"] + sorted(papers_df['Year'].unique().tolist()))
        ...
        with col2:
            ...
            if selected_ay.keyword_frequencies:
                kw_df = pd.DataFrame([
                    ...
                    for kw, freq in list(selected_ay.keyword_frequencies.items())[:20]
                ])
...
def show_data_exploration():
    ...
    if st.button("Export Data Summary as JSON"):
        summary_data = {
            ...
            "total_papers": sum(ay.paper_count for ay in academic_years),
            "total_citations": sum(ay.total_citations for ay in academic_years),
            "year_range": [min(ay.year for ay in academic_years), max(ay.year for ay in academic_years)],
            "yearly_data": [
                {
                    "year": ay.year,
