        ...
        with progress_container:
            ...
            def update_overall_progress(current_step, total_steps, message):
                ...
                overall_status.write(
                    f"**Overall Progress:** {current_step}/{total_steps} - {message}"
                )
            ...
            update_step_progress(
                0, 1, "Processing all papers and building network graph..."
            )
            ...
            update_log(
                f"🕸️ Citation network: {citation_network.number_of_nodes()} nodes, {citation_network.number_of_edges()} edges"
            )

            # Step 4: Initialize TF-IDF
            update_overall_progress(3, 7, "Initializing text analysis...")
            update_step_progress(
                0, 1, "Setting up TF-IDF vectorizer for theme detection..."
            )
            ...
            for i, period in enumerate(periods):
                ...
                update_step_progress(
                    i + 1,
                    total_periods,
                    f"Processing Period {i+1}/{total_periods} ({period.start_year}-{period.end_year})",
                )
                update_log(
                    f"🔍 Analyzing Period {i+1}: {period.start_year}-{period.end_year}"
                )
                ...
                if len(period_papers) < 3:
                    update_log(
                        f"   ⚠️ Insufficient papers ({len(period_papers)}) for network analysis"
                    )
                    continue
                ...
                update_log(
                    f"   🕸️ Subnetwork: {period_subnetwork.number_of_nodes()} nodes, {period_subnetwork.number_of_edges()} edges"
                )
                ...
                update_log(
                    f"   ✅ Period {i+1} completed: '{period_label}' (confidence: {confidence:.3f})"
                )

                # Update results preview
                with results_preview.container():
                    st.write(
                        f"**Completed Periods ({len(characterized_periods)}/{total_periods}):**"
                    )
                    for j, cp in enumerate(characterized_periods):
                        st.write(
                            f"- Period {j+1}: {cp.start_year}-{cp.end_year} - {cp.topic_label} (conf: {cp.confidence:.3f})"
                        )
            ...
            st.success(
                f"✅ Characterization completed in {characterization_time:.2f}s - {len(characterized_periods)} periods characterized!"
            )
...
def create_network_metrics_chart(periods):
    ...
    fig.update_layout(height=400 * rows, title_text="Network Metrics by Period")
...
def show_period_characterization_details(periods):
    ...
    selected_period_idx = st.selectbox(
        "Select Period for Detailed Analysis",
        range(len(periods)),
    ...
    with col1:
        ...
        st.write(
            f"- **Years:** {selected_period.start_year}-{selected_period.end_year}"
        )
        ...
        st.write(
            f"- **Network Stability:** {getattr(selected_period, 'network_stability', 0.0):.3f}"
        )
        st.write(
            f"- **Community Persistence:** {getattr(selected_period, 'community_persistence', 0.0):.3f}"
        )
        st.write(
            f"- **Flow Stability:** {getattr(selected_period, 'flow_stability', 0.0):.3f}"
        )
        st.write(
            f"- **Centrality Consensus:** {getattr(selected_period, 'centrality_consensus', 0.0):.3f}"
        )
    ...
    if (
        hasattr(selected_period, "representative_papers")
        and selected_period.representative_papers
    ):
            ...
            papers_data.append(
                {
                    ...
                    + ("..." if len(paper.get("title", "")) > 80 else ""),
...
def create_network_structure_visualization(periods):
    ...
    selected_period_idx = st.selectbox(
        "Select Period for Network Visualization",
        range(len(periods)),
        format_func=lambda x: period_options[x],
        key="network_viz",
    )
    ...
    with col2:
        clustering = (
            selected_period.network_metrics.get("clustering_coefficient", 0.0)
            if selected_period.network_metrics
            else 0.0
        )
    ...
    with col3:
        modularity = (
            selected_period.network_metrics.get("modularity", 0.0)
            if selected_period.network_metrics
            else 0.0
        )
    ...
    with col4:
        density = (
            selected_period.network_metrics.get("density", 0.0)
            if selected_period.network_metrics
            else 0.0
        )
...
def show_characterization():
    ...
    with col1:
        ...
        if st.session_state.characterized_periods is None:
            st.write(
                "⚠️ **Note:** This stage involves network analysis and LLM calls, which may take several minutes."
            )
            st.write(
                "🔍 **Process includes:** Citation network analysis, network metrics calculation, theme detection, and LLM-based topic labeling."
            )
        else:
            st.write(
                "✅ **Characterization completed!** View results below or re-run to update with new parameters."
            )
    ...
    if st.session_state.characterized_periods is None:
        if not run_characterization_button:
            st.info(
                "📋 Ready to characterize periods. Click 'Start Characterization' to begin."
            )
    ...
    with col2:
        avg_confidence = (
            sum(p.confidence for p in periods) / len(periods) if periods else 0
        )
        st.metric("Avg Confidence", f"{avg_confidence:.3f}")
    with col3:
        avg_network_stability = (
            sum(getattr(p, "network_stability", 0.0) for p in periods) / len(periods)
            if periods
            else 0
        )
                ...
                summary_data.append(
                    {
                        ...
                        "Years": f"{period.start_year}-{period.end_year}",
                        "Topic": (
                            period.topic_label[:30] + "..."
                            if period.topic_label and len(period.topic_label) > 30
    ...
    tab1, tab2, tab3 = st.tabs(
        ["🔍 Period Details", "🕸️ Network Analysis", "📖 Algorithm Info"]
    )
    ...
    with tab3:
        ...
        **Performance Notes:**
        ...
        if "Characterization" in st.session_state.timing_data:
            st.write(
                f"**Last Run Time:** {st.session_state.timing_data['Characterization']:.2f} seconds"
            )
