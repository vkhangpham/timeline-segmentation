...
def create_comprehensive_timeline_chart(result):
    ...
    for i, period in enumerate(result.periods):
        ...
        topic_text = (
            ...
            else period.topic_label or f"Period {i+1}"
        )
    ...
    fig = go.Figure(
        data=[
            go.Sankey(
                ...
                link=dict(
                    source=source, target=target, value=values, color="lightgray"
                ),
            )
        ]
    )

    fig.update_layout(title_text="Topic Evolution Flow", font_size=10, height=400)
...
def show_algorithm_performance_summary():
    ...
    if st.session_state.timing_data:
        ...
        with col2:
            longest_stage = max(
                st.session_state.timing_data.items(), key=lambda x: x[1]
            )
        ...
        with col4:
            if st.session_state.academic_years:
                papers_processed = sum(
                    ay.paper_count for ay in st.session_state.academic_years
                )
...
def show_quality_assessment(result):
    ...
    paper_balance = (
        1 - max(p.total_papers / total_papers for p in periods)
        if total_papers > 0
        else 0
    )
...
def show_final_results():
    ...
    with col4:
        total_years = (
            max(p.end_year for p in result.periods)
            - min(p.start_year for p in result.periods)
            + 1
        )
    ...
    for i, period in enumerate(result.periods):
        with st.expander(
            f"Period {i+1}: {period.start_year}-{period.end_year} - {period.topic_label or 'Unlabeled'}"
        ):
            ...
            with col1:
                ...
                st.write(
                    f"**Duration:** {period.end_year - period.start_year + 1} years"
                )
            ...
            "periods": [
                {
                    ...
                    "keyword_frequencies": dict(
                        list(p.combined_keyword_frequencies.items())[:20]
                    ),
                }
        ...
        st.json(
            {
                ...
                "sample_period": (
                    export_data["periods"][0] if export_data["periods"] else {}
                ),
            }
        )
    ...
    # Success message
    st.success(
        f"🎉 Timeline analysis completed successfully! Identified {len(result.periods)} distinct periods in {result.domain_name.replace('_', ' ')} with {result.confidence:.1%} confidence."
    )
